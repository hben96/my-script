...
data_ip="https://raw.githubusercontent.com/hben96/my-script/main/ip"
checking_sc() {
...
#########################
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/hben96/my-script/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/hben96/my-script/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
# Status ExpiRED Active | Julak Project
Info="(${green}Active${NC})"
Error="(${RED}ExpiRED${NC})"
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(curl https://raw.githubusercontent.com/hben96/my-script/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# julak    
    julak="https://raw.githubusercontent.com/hben96/my-script/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/hben96/my-script/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/hben96/my-script/main/ip | grep $ipsaya | awk '{print $3}')
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${julak}julak/config.json" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${julak}julak/runn.service" >/dev/null 2>&1
    domain=$(cat /etc/xray/domain)
    ...
    curl ${julak}bantur/nginx.conf > /etc/nginx/nginx.conf
...
wget raw.githubusercontent.com/hben96/my-script/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "${julak}limit/limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
...
wget -q -O /etc/ipserver "${julak}bantur/ipserver" && bash /etc/ipserver
}
...
wget -O /usr/sbin/ftvpn "${julak}baku/ftvpn" >/dev/null 2>&1
